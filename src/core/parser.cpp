...
void parse(const char *scene_file_name)
{
  ...
  if (xml_doc.LoadFile(scene_file_name) != tinyxml2::XML_SUCCESS)
    RT3_ERROR(
      std::string{ "The file \"" } + scene_file_name
      + std::string{ "\" either is not available OR contains an invalid RT3 scene provided!" });
...
void parse_tags(tinyxml2::XMLElement *p_element, int level)
{
  ...
  while (p_element != nullptr) {
    ...
    clog << "\n"
         << setw(level * 3) << ""
         << "***** Tag id is `" << tag_name << "`, at level " << level << std::endl;
    ...
    if (tag_name == "camera") {
      ...
      vector<std::pair<param_type_e, string>> param_list{
        ...
        { param_type_e::ARR_REAL, "screen_window" }  // Bounds of the film in screen space. [-1,1] along the shorter image
                             // axis, and its set proportionally along the longer axis.
      };
 ...
 * @param ps_out The `ParamSet` object we need to fill in with parameter information extracted from
 * the XML element.
 */
                      ...
                      ParamSet *ps_out)
{
  ...
  for (const auto &e : param_list) {
    ...
    case param_type_e::VEC3I:
      ...
      break;
    case param_type_e::VEC4F:
      parse_single_COMPOSITE_attrib<real_type, Vector4f>(p_element, ps_out, name);
      break;
...
bool parse_single_COMPOSITE_attrib(tinyxml2::XMLElement *p_element,
                                   ...
                                   string att_key)
{
  // Attribute() returns the value of the attribute as a const char *, or nullptr if such attribute
  // does not exist.
  ...
  if (att_value_cstr) {
    ...
    if (not result.has_value())
      RT3_ERROR(string{ "parse_single_COMPOSITE_attrib(): could not read values for attribute \""
                        + att_key + "\"!" });
                                  ...
                                  string att_key)
{
  ...
  if (att_value_cstr) {
    ...
    // Error check
    if (not result.has_value())
      RT3_ERROR(string{ "parse_array_COMPOSITE_attrib(): could not read values for attribute \""
                        + att_key + "\"!" });
...
/// Parse the XML element `p_element` looking for an attribute `att_key` and extract one or more
/// values into the `ParamSet` `ps`.
template <typename T>
